...
public class StageMap {
	...
	private int AtBlock;
	private int stage;
	public static int NumberOfStage = 5; // This game have 5 stage
	...
	public StageMap(int x, int y, int AtBlock, int stage) throws SlickException {
		this.x = x;
		this.y = y;
	...
	public void DrawSpace(int stage) throws SlickException {
		...
		while (NumOfSpace < drawSpaceMap[stage - 1].length) {
			if (AtBlock == drawSpaceMap[stage - 1][NumOfSpace]) {
				ChkStepOnSpace = true;
				ChkSpace = true;
			}
			NumOfSpace++;
